...
data_save_path = 'D:\\Backup\\DGMIF_data\\002_photobiomodulation\\preprocessed_EEG_proc_2nd_trial\\'
...
base_path = "D:\\Backup\\DGMIF_data\\002_photobiomodulation\\rat_eeg_2nd\\20200924-200924"
...
data_read.epocs.PrtA.data[np.where(data_read.epocs.PrtA.data==6)] = 7
data_read.epocs.PrtA.data[np.where(data_read.epocs.PrtA.data==5)] = 4
...
for onset_time_div in range(0,int(np.floor(len(tmp.trigger_index)/2))):
    ...
    #if data_read.epocs.PrtA.data[onset_time_div] != 7:
    data.time.rising_index = np.concatenate((data.time.rising_index, tmp.rising_index), axis = None)
    
if PW_true_CW_flase_flag == True:
    data.time.rising_index = data.time.rising_index[0:np.int(data.time.rising_index.size/2)]
else:
    data.time.rising_index = data.time.rising_index[np.int(data.time.rising_index.size/2):]
...
plt.figure(2)
for data_ch in range(0, data.info.ch):            
    plt.subplot(np.ceil(data.info.ch/3), 3, data_ch+1)
    plt.imshow(np.flipud(np.squeeze(20*np.log10(abs(data.data.stft_EEGG[data_ch,:,:])))),vmin = vrange[0], vmax = vrange[1], extent =[0,data.info.active_time,0,data.info.fs/2])
    plt.ylim([0, 160])
...
for data_ch in range(0, data.info.ch):            
    plt.subplot(np.ceil(data.info.ch/3), 3, data_ch+1)
    plt.imshow(np.flipud(np.squeeze(20*np.log10(abs(data.data.stft_wavEEGG[data_ch,:,:])))),vmin = vrange[0], vmax = vrange[1], extent =[0,data.info.active_time,0,data.info.fs/2])
...
for data_ch in range(0, data.info.ch):            
    plt.subplot(np.ceil(data.info.ch/3), 3, data_ch+1)
    plt.imshow(np.flipud(np.squeeze(20*np.log10(abs(extracted_noise[data_ch,:,:])))),vmin = vrange[0], vmax = vrange[1], extent =[0,data.info.active_time,0,data.info.fs/2])
    plt.ylim([0, 160])
...
data.time.prt1_onset_PW = data_read.epocs.PrtA.onset[(np.arange(0,np.uint8(np.uint8(data_read.epocs.PrtA.onset.shape[0])/4))*2)]
data.time.prt1_onset_CW = data_read.epocs.PrtA.onset[(np.arange(np.uint8(np.uint8(data_read.epocs.PrtA.onset.shape[0])/4),np.uint8(np.uint8(data_read.epocs.PrtA.onset.shape[0])/2))*2)]
if PW_true_CW_flase_flag == True:
    data.time.prt1_onset = data.time.prt1_onset_PW 
else:
    data.time.prt1_onset = data.time.prt1_onset_CW
...
for data_ch in range(0, data.info.ch):            
    plt.subplot(np.ceil(data.info.ch/3), 3, data_ch+1)
    plt.imshow(np.flipud(np.squeeze(20*np.log10(abs(data.data.stft_epoc_avg[data_ch,:,:])))),vmin = vrange[0], vmax = vrange[1], extent=[-epoc_time_des*10,epoc_time_des*10,0,data.info.fs/2])
    plt.ylim([0, 160])
...
for data_ch in range(0, data.info.ch):            
    plt.subplot(np.ceil(data.info.ch/3), 3, data_ch+1)
    plt.imshow(np.flipud(np.squeeze(20*np.log10(abs(data.data.stft_epoc_div_commref_avg[data_ch,:,:])))),vmin = vrange[0], vmax = vrange[1], extent=[-epoc_time_des*10,epoc_time_des*10,0,data.info.fs/2])
    plt.ylim([0, 160])
    plt.title('STFT-avg, Ch' + str(data_ch))
    plt.ylabel('Hz')
    plt.xlabel('time(0.1s)')
...
filename = data_save_path + file_path + '.pkl'        
