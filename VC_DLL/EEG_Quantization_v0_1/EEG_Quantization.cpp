...
void get_stress_index(unsigned int* raw, float* stress_index)
{
	...
	float* spectrum_r = new float[n_pts * n_channel];
...
void get_quantized_index(unsigned int* raw, float* rslt)
{
	float* delta = new float[n_channel];
	float* theta = new float[n_channel];
	float* alpha = new float[n_channel];
	...
	float depression_index = 0;
	float attention_index = 0;

	memset(rslt, 0, sizeof(float) * 3);
	...
	rslt[0] = stress_index;
	rslt[1] = depression_index;
	rslt[2] = attention_index;
